...
import { roomHelpers, type RoomApiResponse } from '@/lib/api-helpers'
...
export function JoinRoomDialog({ children }: JoinRoomDialogProps) {
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      ...
      const room = (roomsResponse.data as unknown as RoomApiResponse)?.room
      console.log('__________room', room);
      ...
      // Emit room:join event to notify server and get room sync
      console.log('[DEBUG] Socket state:', { socket: !!socket, isConnected, roomId: room.id })
      
      if (socket && isConnected) {
        try {
          // Emit room:join event (client only sends roomId)
          console.log('[DEBUG] About to emit room:join event...')
          socket.emit('room:join', { roomId: room.id })
          
          console.log('[Client] Emitted room:join event for room:', room.id)
        } catch (socketError) {
          console.warn('[Client] Failed to emit room:join event:', socketError)
          // Continue even if socket events fail
        }
      } else {
        console.warn('[DEBUG] Cannot emit room:join - socket not ready:', { socket: !!socket, isConnected })
        
        // Try to wait for socket to connect
        if (socket && !isConnected) {
          console.log('[DEBUG] Waiting for socket to connect...')
          socket.once('connect', () => {
            console.log('[DEBUG] Socket connected, now emitting room:join...')
            socket.emit('room:join', { roomId: room.id })
            console.log('[Client] Emitted room:join event after waiting for connection')
          })
        }
      }
